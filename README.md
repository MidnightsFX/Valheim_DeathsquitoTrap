...
A compressed file with the binaries is created in `<DvergerSecretDefenses>\Packages`ready for upload to ThunderStore.
...
A Unity project is included in this repository under `<DvergerSecretDefenses>\JotunnModUnity`.
...
4. Compile the project. This copies all assemblies into `<DvergerSecretDefenses>\JotunnModUnity\Assets\Assemblies`. Don't open Unity yet before this step, it will remove assembly references.
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<DvergerSecretDefenses>\JotunnModUnity\Assets\Assemblies`. <br />
